...
# https://dsgarage.netlify.app/bootcamp/1.5-m1-data-subsetting-and-manipulation/
...
# Notice we have to place non-consecutive index values in the c() function
my_vector[ c(5, 9)] 
...
# Flash challenge: 
# Name the columns of my_matrix "a", "b", "c", "d" with colnames()
...
# Flash challenge: 
# Draw a new barplot by recycling the code above
# This time, add error bars showing on both the top and the 
# bottom of the mean values




## 1.5.5 Practice exercises ####
# For the following exercises, use the trees dataset, 
# which has Girth, Height and Volume variables for 
...
### 01 ####
# Show code to calculate the mean Girth of Black Cherry trees 
# with Height less than 75 ft.


### 02 ####
# Use help(cut) and then use the cut() function to create a new 
# factor variable based on the Height numeric variable in the 
# trees dataset. Try setting the breaks argument to 2 or 3. 
...
### 03 ####
# Using the new factor from question 2, use aggregate() to 
# calculate the mean and standard deviation of all three 
# variables in the trees data. Show your code and report the 
# results to 2 decimal points of accuracy.

### 04 ####
# Show the code using which() and boolean phrases as appropriate 
# to find the rows in the trees dataset where Girth is 
# higher than 11 and Height is lower than 75.
...
# Use aggregate() to calculate the mean of volume for each population 
# (hint: you may need to use help for the FUN functions 
# involved and pay close attention to your data frame.).
